# Populates a dynamic drop down with the Organizations from Satellite.
#
@DEBUG = false
...
begin
  ...
  $evm.log(:info, "organization_index = #{organization_index}") if @DEBUG
  ...
  dialog_field["values"]        = Hash[ *organization_index['results'].collect { |item| [item['id'], item['name']] }.flatten ]
end
