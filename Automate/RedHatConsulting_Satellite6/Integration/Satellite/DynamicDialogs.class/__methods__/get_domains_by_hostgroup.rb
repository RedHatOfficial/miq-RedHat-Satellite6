# Populates a dynamic drop down with the domain from a Satellite Hostgroup based on the selected Hostgroup.
#
@DEBUG = false
...
begin
  ...
  satellite_hostgroup_id = get_param(:dialog_satellite_hostgroup_id)
  $evm.log(:info, "satellite_hostgroup_id => #{satellite_hostgroup_id}") if @DEBUG
  
  # if no location tags selected then hide dialog element
  return_dialog_element(false, {}) if satellite_hostgroup_id.empty?
  
  # if no hostgroup selected the return error
  return_dialog_element(true, { nil => "ERROR: Please select a Hostgroup" }) if satellite_hostgroup_id.to_i == 0
  
  # verify a locaiton is selected for the given index
  location_index = get_param(:location_index)
  return_dialog_element(false, {}) if location_index >= satellite_hostgroup_id.length
  ...
  # search for the hostgroup id
  hostgroups_search_string = "id = #{satellite_hostgroup_id}"
  ...
  return_dialog_element(true, { nil => "ERROR: Could not find Satellite Domains for Satellite Hostgroup ID <#{satellite_hostgroup_id}>" }) if hostgroups_index['results'].empty?
  
  # return results
  return_dialog_element(
    true,
    Hash[ *hostgroups_index['results'].collect { |item| [item['domain_name'], "#{item['domain_name']} (#{item['title']})" ] }.flatten ],
    hostgroups_index['results'][0]['name']
  )
end
